...
const { GetManifest, Download } = require('./util');
...
const DownloadBinary = (
  ...
  Flow(function*(cb) {
    ...
    const url = `${mirror}/${version}/nwjs${
      flavor == 'normal' ? '' : '-' + flavor
    }-${version}-${target}${EXTENSIONS[target]}`;

    debug('url:', url);

    var [err, fromCache, path] = yield Download(
      url,
      {
        cachePrefix: 'binary',
    ...
    if (unzip) {
      createReadStream(path)
        ...
        .on('close', () => callback(err, fromCache, path));
    } else {
...
Object.assign(module.exports, {
  ...
  DownloadFFmpeg,
  // Deprecated.
  GetManifest: deprecate(
    GetManifest,
    'GetManifest is deprecated, use util.GetManifest instead.'
  ),
});

...
const { basename, join, resolve, isAbsolute } = require('path');
...
const GetManifest = (callback) => {
  ...
  Flow(function*(cb) {
    ...
    if (cacheExists) {
      ...
      useCache =
        !err && Date.now() - cacheStat.ctime.getTime() < EXPIRY_MANIFEST_CACHE;
...
const Download = (
  ...
  Flow(function*(cb) {
    let path = cachePrefix
      ? join(DIR_CACHES, cachePrefix + '-' + basename(url))
      : temp.path();
    ...
    var [err, res, body] = yield wrapProgress(
      request(
        url,
        {
          encoding: null,
      ...
      if (showProgressbar) {
        ...
        progressbar.total = progress.size.total
          ? progress.size.total
          : progressbar.curr;
        ...
        progressbar.tick({
          ...
          ETA: progress.time.remaining
            ? progress.time.remaining.toFixed(2) + 's'
            : '-',
        });
      }
